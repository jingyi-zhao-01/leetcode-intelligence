...
### 1. lifecycle 分成七段: activate, persist, exact-recall, similar-recall, cache, hydrate, mount-hint
...
4. `similar-recall`
   - 当前题目已经拿到 `title / difficulty / question_content / topic_tags` 这些 descriptor 后，再基于持久化的 similarity metadata 检索历史类似题
   - 这里返回的是“值得类比的历史题”，不是“当前题一定同构”的断言
5. `cache`
...
### 3.5. persist 时自动补写 similarity metadata

每条持久化 session record 除了精确 recall 所需的 `title_slug / end_reason / latest_failure_status` 之外，还会自动写入一组“解法迁移性”字段。
...
这些字段由 submission service 在 persist 时自动提取，输入来源是:
...
- `question_content`
- `topic_tags`
- latest failure summary / annotations
- 当前 session 里保留下来的 thought process

约束是:

- 这些字段用于 similar recall，不参与 active session source of truth
- 提取结果必须是 bounded 的短 tags / summary，不能把大段题面原样再塞进 metadata
- 即使某次 session 只拿到部分上下文，也允许只写出部分 similarity fields

### 3.6. similar recall 不是 related-questions 镜像，而是按 metadata overlap 评分

similar recall 不按 `title_slug` 精确命中，而是:

- 为当前题构造 query profile
- 从当前用户 / app / agent 下的历史 `leetcode_session_record` 里取候选
- 排除当前 `title_slug`
- 按下面这些 overlap 评分:
  - `pattern_tags`
  - `state_traits`
...
它的目标是找出“这题之前你做过哪些值得迁移思路 / 易错点的历史题”，不是复刻 LeetCode 官方的相似题列表。
...
- 如果 on-mount 请求已经带上当前题的 `title / difficulty / question_content / topic_tags`，服务端也会顺手返回类似题摘要
...
3. `Submission Service Similar Problem Recall`
4. 当前 session 的 `sessionMemory`
5. 当前 chat 的 `companionMemory`
...
`Similar Problem Recall` 的职责是:

- 告诉 companion 你以前做过哪些“解法上值得类比”的题
- 暴露这些历史题共享的 pattern / state trait / domain signals
- 带上这些题当时的 failure reason / stuck point / thought process
...
`Similar Problem Recall` 不应该做的事:

- 把“相似”说成“同题”或“同解”
- 只因为标题像就返回高优先级结果
- 把完整旧 session dump 当作类比证据重新塞进 prompt

因此当前 recall 渲染策略是:

- 保留结构化 session-level 摘要
- 在 render 前先按 `run_id`（没有时退回 `activated_at`）归一化 raw recalled records
  - 同一次 solve run 里由 `failure_analysis`、`accepted_restart` 等路径写出的多条 raw record 会先合并成一个历史 session 视图
  - `record_count`、on-mount summary、companion hidden context 共享同一份归一化结果，避免不同视图各自解释 raw records 导致数据不一致
