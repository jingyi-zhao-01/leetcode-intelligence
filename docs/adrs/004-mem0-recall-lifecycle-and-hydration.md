...
### 1. lifecycle 分成五段: activate, persist, recall, hydrate, mount-hint
...
### 5. 题目 on-mount 提示复用同一份服务端 recall，不在客户端重建语义
...
### 5. companion prompt 总是把 recall 当作历史摘要，而不是实时事实
...
### 6. recall 和当前 active session 的职责边界必须保持清楚
